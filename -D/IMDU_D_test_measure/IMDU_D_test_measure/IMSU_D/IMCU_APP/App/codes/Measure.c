...
static UCHAR Measure_GetChannelAdc3Value(UCHAR ucChannel,UINT16* pData)
{
	UCHAR ucRTN = FALSE;
	ucRTN= dv_ADC_3.fGetAdcValue(ucChannel,pData);
	return ucRTN;
}
...
static void GetVFAnalog(void)
{
	...
	if(gMeasureInfo.ucSlowMeasureStep[0] == 0)
	{
		dv_Adc.ucStartMeasureFlag = 1;
		gMeasureInfo.ucSlowMeasureStep[0] =1;
	}
	else if(gMeasureInfo.ucSlowMeasureStep[0] == 1)
	{
		if(dv_Adc.ucStartMeasureFlag == 0)
		{
			...
			dv_ADC_3.fADWorkPortSelect(0);	
			dv_ADC_3.fStartMeasure(dv_ADC_3.u32OneLoopValue,10);
			gMeasureInfo.ucIsMeasureOneLoop = TRUE;
			gMeasureInfo.ucSlowMeasureStep[0] = 0;
		}

	}
	if(gMeasureInfo.ucSlowMeasureStep[1] == 0)
	{
		dv_ADC_3.fADWorkPortSelect(0);	
		dv_ADC_3.fStartMeasure(dv_ADC_3.u32OneLoopValue,10);
		dv_ADC_3.ucStartMeasureFlag = 1;
		gMeasureInfo.ucSlowMeasureStep[1] =1;
	}
	else if(gMeasureInfo.ucSlowMeasureStep[1] == 1)
	{
		if(dv_ADC_3.ucStartMeasureFlag == 0)
		{
			RTN = Measure_GetChannelAdc3Value(0,&u16temp);
			if(RTN == TRUE)
			{
				ftemp = 2.5*((float)u16temp)/4096*5/2;
			}
			gMeasureInfo.ucSlowMeasureStep[1] = 0;
		}
