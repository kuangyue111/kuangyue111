...
snrslv(struct req_desc *rqdp)
{
	...
	for (bl = rqdp->bindlf, i = 1; bl; bl = bl->sb_next, i++) {
		...
		if ((np = getmib(&bl->sb_oid)) == 0) {
				...
				struct 	oid stTempOID;

				memcpy(&stTempOID, &bl->sb_oid, sizeof(stTempOID));
				stTempOID.len += 1;
				stTempOID.id[stTempOID.len - 1] = 0;

				if ((np = getmib(&stTempOID)) == 0)
				{
					memcpy(&stTempOID, &bl->sb_oid, sizeof(stTempOID));
					stTempOID.id[stTempOID.len - 1] = 1;

					if ((np = getmib(&stTempOID)) == 0)
					{
						//Modified by hulw 090507 to support SNMP TRAP TABLE
						/*seterr(SERR_NO_SUCH);				
						return SYSERR;*/
						memcpy(&stTempOID, &bl->sb_oid, sizeof(stTempOID));
						stTempOID.len += 1;
			...
			seterr(SERR_NO_SUCH);
		     	...
		     	return SYSERR;
						...
						else if (rqdp->reqtype == PDU_GET)
						{
							if (np->mi_func != 0) {
								(*np->mi_func)(bl, np, SOP_GET);
								continue;
							}
							else
							{
