...
#define HARDWARE_IS_1000V						0x5A
#define HARDWARE_ISNT_1000V						0xA5
...
typedef	void(*INUINT16_2PUCHAR_INTERFACE_PROC)(UCHAR *data,UINT16 data2,UCHAR *Pdata);
...
typedef struct tagdv_USB
{
	...
	INUINT16_2PUCHAR_INTERFACE_PROC fUSBWriteData;
...
typedef struct tagdv_DO
{
	INUCHAR_INTERFACE_PROC   fDO1Ctrl;
	INUCHAR_INTERFACE_PROC   fDO2Ctrl;
}dv_DO_TypeDef;
...
typedef struct tagCANCommData
{
	...
	int			nOnlineTotalGrp; //實際在線的各種模塊的組數,必须为0,1,2
...
typedef union tagSetData
{
	...
	struct sElemetData
	{
		...
		UCHAR   ucDoCtrlBoard;						/*序号:8,DO控制板*/	
		UCHAR   ucNetEnable;						/*序号:9,网络使能*/	
		UCHAR   ucDHCPEnable;						/*序号:10,网络使能*/	
		UCHAR   ucSelfIP[4];						/*序号:11-14,本地IP*/	
		UCHAR   ucSubMask[4];						/*序号:15-18,子网掩码*/	
		UCHAR   ucGateWay[4];						/*序号:19-22,默认网关*/	
		UCHAR   ucServerIP[4];						/*序号:23-26,服务器IP*/	
		UCHAR   ucNetPort[2];						/*序号:27-28,网络端口*/	
		UCHAR   ucMACAddress[6];						/*序号:29-34,网络端口*/	
		UCHAR	u8Data[4061];						/*序号:35-4095,系统类型*/
...
struct ChargeRecordFlah
{
	...
	UINT8 u8StartTime[7];				//3 老化开始时间			7		
	UINT8 u8EndTime[7];					//4 老化结束时间			7		
	UINT8 u8EVSerialNum[32];			//5 测试机柜序列号			32		
	float fUserSetVolt;					//6 用户设置的充电电压		4		
	float fUserSetCurr;					//7 用户设置的充电电流		4		
	float fEVOutputMaxVolt;				//8 桩上报的充电电压最大值	4		
	float fEVOutputMinVolt;             //9 桩上报的充电电压最小值	4		
	float fEVOutputMaxCurr;				//10桩上报的充电电流最大值	4		
	float fEVOutputMinCurr;				//11桩上报的充电电流最小值	4		
	float fMeasuerMaxVolt;				//12 工装测量最大电压		4		
	float fMeasuerMinVolt;				//13 工装测量最小电压		4		
	float fMeasuerMaxCurr;				//14 工装测量最大电流		4		
	float fMeasuerMinCurr;				//15 工装测量最小电流		4		
	float fCC2Volt;						//16 工装测量的CC2电压		4		
	float fBMSVolt;						//17工装测量的辅助源电压	4		
	UINT16 u16UserSetTime;				//18用户设置的充电时长		2		
	UINT16 u16TestTime;					//19 老化实际测试时间		2		
	UINT8 u8TestResult;					//20 老化测试结果			1		
	UINT8 u8TestFailReason;             //21 老化测试失败原因		1		
	UINT8 u8Reserve[2];					//22 老化测试失败原因		1		
};
